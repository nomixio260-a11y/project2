...
test("CivSystem: 拡張ループの解消 — 開拓者は領有でき保持できる土地だけを目指す", () => {
  ...
  const A = civ.foundAt(20, 20);
  const k = civ.kingdoms[A];
  const h = { x: 20.5, y: 20.5, kid: A };

  // 支配圏内の無主の草原 → 目標になる。
  let t = civ._expandTarget(h, k, w);
  ...
  const h2 = { x: 70.5, y: 20.5, kid: A }; // その近くに立つ開拓者
...
test("CivSystem: 余暇・文化 — 充足した人々は遊び・祭り・礼拝など多彩に過ごす、危機時は生業優先", () => {
  const Game = loadCore({ mapWidth: 40, mapHeight: 40, seed: 4242 });
  const w = new Game.World(40, 40); w.terrain.fill(Game.TERRAIN.GRASS);
  if (w.fertility) w.fertility.fill(0.6);
  const civ = new Game.CivSystem(w, { markTerritoryDirty() {}, markDirty() {} });
  Game.state = Game.state || {}; Game.state.civ = civ;
  const A = civ.foundAt(20, 20);
  const k = civ.kingdoms[A];
  const ROLE = Game.ROLE;
  // 余暇先となる施設を備える（神殿・市・酒場・記念碑）。
  k.cities[0].buildings.push(
    { x: 21, y: 20, t: 4, lvl: 1, cond: 1 }, // 神殿
    { x: 19, y: 20, t: 7, lvl: 1, cond: 1 }, // 市場
    { x: 20, y: 21, t: 14, lvl: 1, cond: 1 },// 酒場
    { x: 20, y: 19, t: 11, lvl: 1, cond: 1 } // 記念碑
  );
  civ._recountFacilities(k);
  k.faith = 0.6; k.unrest = 0;

  function trial(setup) {
    const states = {};
    for (let i = 0; i < 400; i++) {
      const h = { x: 20.2, y: 20.2, age: 1000, food: 0.9, mood: 0.9, kid: A,
        role: ROLE.FARMER, wit: 1.2, creat: 1.2, synSoc: 1, aspire: 0, skill: 0.5, mind: 0.3, joy: 0.3, social: 0 };
      setup(h);
      const did = civ._leisure(h, k, w, 20, 20);
      if (did) states[h.state] = (states[h.state] || 0) + 1;
    }
    return states;
  }

  // 平時・充足した大人 → 余暇に出る（祭り17/礼拝18/市19/物見21 のいずれかが現れる）。
  k.wars = {};
  const adult = trial(() => {});
  const leisureSum = (adult[17]||0)+(adult[18]||0)+(adult[19]||0)+(adult[21]||0);
  assert.ok(leisureSum > 20, "充足した大人は余暇に出るはず: " + JSON.stringify(adult));
  assert.ok(Object.keys(adult).length >= 2, "余暇の行動が多彩に分かれるはず: " + JSON.stringify(adult));

  // 子供 → 遊ぶ（state 20）。
  const kids = trial((h) => { h.age = 80; });
  assert.ok((kids[20] || 0) > 50, "子供は遊ぶはず: " + JSON.stringify(kids));

  // 戦時 → 大人は余暇に出ない（生業・防衛優先）。
  k.wars = { 99: 0 };
  const war = trial(() => {});
  assert.equal(Object.keys(war).length, 0, "戦時に大人は余暇に出ない: " + JSON.stringify(war));

  // 空腹 → 余暇より食料確保を優先（false を返す）。
  k.wars = {};
  const hungry = trial((h) => { h.food = 0.3; });
  assert.equal(Object.keys(hungry).length, 0, "空腹時は余暇に出ない: " + JSON.stringify(hungry));

  // 高い不満（騒然とした国）→ 祭りは影をひそめる。
  k.unrest = 95;
  const unrest = trial(() => {});
  assert.equal(Object.keys(unrest).length, 0, "高い不満では余暇に出ない: " + JSON.stringify(unrest));
});

test("CivSystem: 放浪者は荒野の扶養力に縛られ、際限なく増殖しない（領内・過密では繁殖しない）", () => {
  const Game = loadCore({ mapWidth: 40, mapHeight: 40, seed: 321 });
  const w = new Game.World(40, 40); w.terrain.fill(Game.TERRAIN.GRASS);
  if (w.fertility) w.fertility.fill(1);
  const civ = new Game.CivSystem(w, { markTerritoryDirty() {}, markDirty() {} });
  Game.state = Game.state || {}; Game.state.civ = civ;
  Game.state.clock = { year: 1 };

  function mkNomad(x, y) {
    return { x: x, y: y, hx: 0, hy: 0, kid: 0, clan: 0, age: 1000, food: 1,
      role: 0, state: 0, gx: x|0, gy: y|0, repro: 0, social: 0, alive: true,
      wit: 1, aspire: 0, lx: 0.5, ly: 0.5 };
  }
  function tryRepro(group) {
    civ.people = group; civ._births = [];
    civ._buildGrid();
    for (const h of group) civ._tryReproduceNomad(h);
    return civ._births.length;
  }

  // 無所属の荒野に少数の放浪者（伴侶あり）→ 子をなす。
  w.owner.fill(0);
  const few = [mkNomad(10.2, 10.0), mkNomad(10.4, 10.1)];
  assert.ok(tryRepro(few) > 0, "荒野の小集団は子をなすはず");

  // 同じ場所に過密（扶養力超過）→ 繁殖しない。
  const many = [];
  for (let i = 0; i < 20; i++) many.push(mkNomad(20 + (i % 3) * 0.2, 20 + ((i / 3) | 0) * 0.2));
  assert.equal(tryRepro(many), 0, "過密な放浪者は増えないはず（荒野の扶養力）");

  // 国の領内をさまよう群れ → そこでは繁殖しない（加入・建国が筋）。
  w.owner.fill(0);
  for (let y = 0; y < 40; y++) for (let x = 0; x < 40; x++) w.owner[y * 40 + x] = 1; // 全土を所有
  assert.equal(tryRepro(few), 0, "国の領内では放浪者は繁殖しないはず");
});

test("CreatureSystem: 捕食者は獲物がいれば辺境から再導入され、絶滅したままにならない", () => {
  const Game = loadCore({ mapWidth: 50, mapHeight: 50, seed: 99 });
  const w = new Game.World(50, 50); w.terrain.fill(Game.TERRAIN.GRASS); // 全土が可食地
  const ents = new Game.Entities(2000);
  const cr = new Game.CreatureSystem(ents, w, { markDirty() {} });
  const S = Game.SPECIES;
  // 草食を十分に置き、捕食者は0（絶滅状態）。
  for (let i = 0; i < 40; i++) ents.spawn(S.HERBIVORE, 5 + (i % 20), 5 + ((i / 20) | 0), 0.8);
  let pred0 = 0; for (let i = 0; i < ents.count; i++) if (ents.alive[i] && ents.type[i] === S.PREDATOR) pred0++;
  assert.equal(pred0, 0, "前提: 捕食者は絶滅している");
  // 移入は確率的なので、複数回試行すれば必ず番が戻る。
  let spawned = false;
  for (let t = 0; t < 40 && !spawned; t++) {
    cr._replenish(w);
    let pred = 0; for (let i = 0; i < ents.count; i++) if (ents.alive[i] && ents.type[i] === S.PREDATOR) pred++;
    if (pred > 0) spawned = true;
  }
  assert.ok(spawned, "獲物が十分なら捕食者が再導入されるはず");
});

test("CivSystem: 基礎死亡率は前近代の死亡曲線（乳幼児高・成人低・老年で上昇、栄養と生命力で増減）", () => {
  const Game = loadCore({ mapWidth: 20, mapHeight: 20, seed: 7 });
  const w = new Game.World(20, 20); w.terrain.fill(Game.TERRAIN.GRASS);
  const civ = new Game.CivSystem(w, { markTerritoryDirty() {}, markDirty() {} });
  const k = { facilities: { temple: 0 } };
  const mk = (age, food, vigor) => ({ age: age, food: food == null ? 1 : food, vigor: vigor == null ? 1 : vigor });

  const infant = civ._baseMortality(mk(5), k);
  const child = civ._baseMortality(mk(150), k);
  const adult = civ._baseMortality(mk(1000), k);
  const elderYoung = civ._baseMortality(mk(2600), k);
  const elderOld = civ._baseMortality(mk(3500), k);

  assert.ok(infant > child, "誕生直後が最も死にやすい: " + infant + " > " + child);
  assert.ok(child > adult, "乳幼児期は成人期より死にやすい: " + child + " > " + adult);
  assert.ok(elderOld > adult, "老年期は成人期より死にやすい: " + elderOld + " > " + adult);
  assert.ok(elderOld > elderYoung, "老いるほど死にやすい（漸増）: " + elderOld + " > " + elderYoung);

  // 栄養不良は死亡率を上げ、生命力は下げる。
  assert.ok(civ._baseMortality(mk(1000, 0.2), k) > adult, "栄養不良で死にやすい");
  assert.ok(civ._baseMortality(mk(1000, 1, 1.3), k) < adult, "生命力が高いほど死ににくい");
  // 神殿（衛生）は死亡率を下げる。
  assert.ok(civ._baseMortality(mk(1000), { facilities: { temple: 5 } }) < adult, "神殿の普及で死ににくい");
});

test("CreatureSystem: 形質のトレードオフ — 俊敏な個体ほど基礎代謝が高くエネルギーを多く消費する", () => {
  const Game = loadCore({ mapWidth: 24, mapHeight: 24, seed: 5 });
  const w = new Game.World(24, 24); w.terrain.fill(Game.TERRAIN.SAND); // 非可食・非水（採食も給水もできない）
  const ents = new Game.Entities(50);
  const cr = new Game.CreatureSystem(ents, w, { markDirty() {} });
  const S = Game.SPECIES;
  // 俊敏さ以外は同一の2個体（繁殖しないよう energy は低め）。
  const slow = ents.spawn(S.HERBIVORE, 6, 6, 0.7, 1.0, 0.7, 1.0, 1.0);
  const fast = ents.spawn(S.HERBIVORE, 18, 18, 0.7, 1.0, 1.3, 1.0, 1.0);
  for (let t = 0; t < 30; t++) cr.tick(w);
  assert.ok(ents.energy[fast] < ents.energy[slow],
    "俊敏な個体ほど代謝が高く消耗する（速さはタダではない）: fast " + ents.energy[fast].toFixed(3) + " < slow " + ents.energy[slow].toFixed(3));
});

test("CivSystem: 帝国の分裂 — 多都市の国は地方都市が領土・住民を伴って独立できる", () => {
  const Game = loadCore({ mapWidth: 60, mapHeight: 60, seed: 17 });
  const w = new Game.World(60, 60); w.terrain.fill(Game.TERRAIN.GRASS);
  const civ = new Game.CivSystem(w, { markTerritoryDirty() {}, markDirty() {} });
  Game.state = Game.state || {}; Game.state.civ = civ;
  Game.state.clock = { year: 1 };
  const A = civ.foundAt(10, 10);
  const k = civ.kingdoms[A];
  // 首都から遠い地方都市を足し、その周囲を領有させる。
  k.cities.push({ x: 45, y: 45, capital: false, level: 2, buildings: [] });
  for (let y = 35; y <= 55; y++) for (let x = 35; x <= 55; x++) { if (x>=0&&y>=0&&x<60&&y<60){ w.owner[y*60+x] = k.id; } }
  // その地方に住民を配置（独立に随伴する）。
  for (let i = 0; i < 8; i++) civ.people.push({ x: 45+(i%3)*0.3, y: 45, hx:0, hy:0, kid: k.id, clan: 5, age: 1000, food: 1, role: 1, state: 0, gx:45, gy:45, repro: 0, social: 0, alive: true, home: { x: 45, y: 45 } });
  k.humanCount += 8; k.roleCount[1] += 8;

  const nKBefore = civ.kingdoms.length;
  const citiesBefore = k.cities.length;
  civ._rebellion(k);
  assert.equal(civ.kingdoms.length, nKBefore + 1, "独立で新たな国家が生まれるはず");
  assert.equal(k.cities.length, citiesBefore - 1, "母国は地方都市を失うはず");
  const nk = civ.kingdoms[civ.kingdoms.length - 1];
  assert.ok(nk.alive && nk.tileCount > 0, "独立国は領土を得るはず: " + nk.tileCount);
  assert.ok(nk.cities[0].x === 45 && nk.cities[0].y === 45, "最遠の地方都市が独立国の首都になるはず");
});

test("CivSystem: 年代記には傑出した人物のみを刻む（地域の名士で洪水にしない）", () => {
  const Game = loadCore({ mapWidth: 24, mapHeight: 24, seed: 3 });
  const w = new Game.World(24, 24); w.terrain.fill(Game.TERRAIN.GRASS);
  const civ = new Game.CivSystem(w, { markTerritoryDirty() {}, markDirty() {} });
  Game.state = Game.state || {}; Game.state.civ = civ;
  Game.state.clock = { year: 1 };
  const A = civ.foundAt(12, 12);
  const k = civ.kingdoms[A];

  function mkPerson(prestige) {
    return { x: 12.2, y: 12.2, kid: A, clan: 7, age: 1500, food: 1, role: 1, state: 0,
      prestige: prestige, skill: 0.5, wit: 1, vigor: 1, mood: 0.6, joy: 0.3, social: 0,
      lx: 0.5, ly: 0.5, culture: 0.5, alive: true, bonds: [] };
  }
  function chronicleCountAfter(prestige) {
    const h = mkPerson(prestige);
    civ.people = [h]; civ._buildGrid();
    const before = civ.events.length;
    civ._socialize(h, k);
    const logged = civ.events.slice(before).some(e => /名を馳せ/.test(e.text));
    return { logged: logged, famed: !!h._famed, chronicled: !!h._chronicled };
  }

  // 閾値ちょうど上（地域の名士）: 視覚的標識は付くが年代記には刻まれない。
  const minor = chronicleCountAfter(7);
  assert.ok(minor.famed, "閾値超えで名士の標識は付く");
  assert.ok(!minor.logged, "地域の名士は年代記に刻まれない（洪水防止）");

  // 傑出した名声（閾値×eminence 以上）: 年代記に刻まれる。
