...
(function (Game) {
  ...
  Game.config = {
    ...
    sim: {
      ...
      // 水循環（hydrology）: 湿度は降雨で増え、蒸発で乾く「動的な収支」。各タイルの湿度は
      //   生成時の気候（moistureBase）を基準に、長期気候(wetness)と季節・気温による蒸発で
      //   絶えず増減する。水辺は染み出しで潤い、高温・夏ほど早く乾く。これで雨蔭・旱・緑化
      //   が湿度を通じて植生・火災・農業へ波及する（系全体の現実性の土台）。
      hydroRate: 0.06,      // 湿度が基準値へ緩和する速さ（蒸発・浸透の時定数）
      hydroClimW: 0.25,     // 長期気候(wetness)が基準湿度を押し引きする強さ
      evapWarmW: 0.5,       // 気温が蒸発（基準湿度の低下）を強める強さ
      evapSeasonW: 0.35,    // 夏は蒸発が強く冬は弱い（季節差）
      seepMoisture: 0.55,   // 水辺タイルが保つ最低湿度（染み出し）
    },
