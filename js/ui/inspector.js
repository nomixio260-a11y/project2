...
(function (Game) {
  ...
  const ACT = {
    ...
    17: "祭り・団欒を楽しむ", 18: "祈りを捧げる", 19: "市を見て回る",
    20: "遊んでいる", 21: "名所・自然を巡る",
  };
  const UPDATE_MS = 220;
  ...
  Inspector._render = function (dead) {
    ...
    this.bodyEl.innerHTML =
      ...
      (function () {
        // 街の発展: 建物の平均段階(普請で育つ)と整備度(状態)を集計して示す。建設中は別枠で数える。
        let n = 0, lvlS = 0, condS = 0, sites = 0;
        if (k.cities) for (let c = 0; c < k.cities.length; c++) {
          const bs = k.cities[c].buildings; if (!bs) continue;
          for (let i = 0; i < bs.length; i++) {
            if (bs[i].site) { sites++; continue; }
            n++; lvlS += (bs[i].lvl || 1); condS += (bs[i].cond == null ? 1 : bs[i].cond);
          }
        }
        if (!n && !sites) return "";
        const lvl = n ? lvlS / n : 1, cond = n ? condS / n : 1;
